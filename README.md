...
## 6. Tasks set to deploy and use scripts on every Game

Each game can be deployed using:

```bash
npx hardhat deployWithArgs --contract-number <CONTRACT NUMBER>
```

Once you get the contract address of the deployed contract, you may launch win scripts with:

```bash
npx hardhat winWithArgs --contract-number <CONTRACT NUMBER> --contract-address <CONTRACT ADDRESS>
```
